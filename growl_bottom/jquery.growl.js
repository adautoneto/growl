...
function notify(title,message,image,priority) {
	...
	if (jQuery.growl.settings.closeButton)
	{
		
		$(jQuery.growl.settings.closeButtonTemplate).prependTo(notice)
			.click(function(){
    ...
    if (jQuery.growl.settings.hideFlash)
    {
        // hide flash elements
        $("object").hide();
    }
...
var noticeRemove = function(notice){
	...
	if (!$(".notice").length && jQuery.growl.settings.hideFlash)
		$("object").show();
}
